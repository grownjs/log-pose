    ...
    if (out && current !== false && allowed <= current) {
      if (format !== null) {
        out.write(format !== false
          ...
          : `${$.puts.apply(null, arguments)}${s || ''}`);
      } else {
        out.write(`${p ? `${p}: ` : ''}${
          $.style($.puts.apply(null, arguments), depth, cliWidth({ stdout: out }))
        }${s || ''}`);
      }
    }
...
function status(type, ctx, cb) {
  ...
  function begin() {
    ...
    if (ok) {
      ...
      interval = setInterval(() => {
        if (ctx.type === false) {
          this.printf('\b{% gray %s %s %}\r\r', _.CHARS[i], ctx.source);
        } else {
          this.printf('\b  {% pad.gray %s %} {% gray %s %s %}\r\r', ctx.prefix || ctx.type, _.CHARS[i], ctx.source);
        }
  ...
  function end(_error) {
    ...
    if (ok) {
      ...
      if (err || _error) {
        if (ctx.type === false) {
          this.printf('\r\r{% fail %s %} %s\n', ctx.source || ctx.target, ms);
        } else {
          this.printf('\r\r  {% pad.gray %s %} {% fail %s %} %s\n', ctx.prefix || ctx.type,
